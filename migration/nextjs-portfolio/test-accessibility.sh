...
# Check if server is running (try ports 3000 and 3001)
PORT=3000
if ! curl -s http://localhost:3000 > /dev/null; then
    if ! curl -s http://localhost:3001 > /dev/null; then
        echo "❌ Error: Development server not running on port 3000 or 3001"
...
URLS=(
    ...
    "http://localhost:$PORT/projects/ecommerce-platform-redesign"
)

# Run pa11y tests
for url in "${URLS[@]}"; do
    page_name=$(echo $url | sed "s|http://localhost:$PORT||" | sed 's|/|-|g' | sed 's|^-||')
    if [ -z "$page_name" ]; then
        page_name="home"
    fi
...
// URLs will be replaced by the shell script
const urls = [
    'http://localhost:PORT_PLACEHOLDER',
    'http://localhost:PORT_PLACEHOLDER/about',
    'http://localhost:PORT_PLACEHOLDER/projects',
    'http://localhost:PORT_PLACEHOLDER/contact',
    'http://localhost:PORT_PLACEHOLDER/projects/ecommerce-platform-redesign'
];
...
async function runAxeTests() {
    ...
    for (const url of urls) {
        ...
        try {
            ...
            const pageName = url.replace('http://localhost:PORT_PLACEHOLDER', '') 
                .replace(/\//g, '-')
                .replace(/^-/, '') || 'home';
...
sed -i '' "s/PORT_PLACEHOLDER/$PORT/g" axe-test.js
